...
enum
{
    pipeline = 0, source, caps, encoder, protocol, sink
};
...
static gboolean periodic_msg_handler(struct stream_info *si)
{
    ...
    if (si->msg_rate > 0)
    {
        ...
        g_object_get(
            G_OBJECT(si->stream[protocol]), "stats", &stats,
            NULL
        );
        if (stats)
        {
            g_print(
                "General RTSP Stats   : %s\n",
 ...
 */
static void media_configure_handler(
...
)
{
    ...
    si->stream[source] = gst_bin_get_by_name(
        GST_BIN(si->stream[pipeline]),
        "source0"
    );
    si->stream[caps] = gst_bin_get_by_name(
        GST_BIN(si->stream[pipeline]),
        "caps0"
    );
    si->stream[encoder] = gst_bin_get_by_name(
        GST_BIN(si->stream[pipeline]),
        "enc0"
    );
    ...
    g_object_set(
        si->stream[encoder], "quant-param", si->curr_quant_lvl,
        NULL
    );
    ...
    g_object_set(
        si->stream[protocol], "config-interval",
...
static void change_quant(struct stream_info *si)
{
    ...
    {
        g_print(
            "[%d]Changing quant-lvl from %d to %d\n", si->num_cli,
            c, si->curr_quant_lvl
        );
        g_object_set(
            si->stream[encoder], "quant-param",
...
static void change_bitrate(struct stream_info *si)
{
    ...
    {
        g_print(
            "[%d]Changing bitrate from %d to %d\n", si->num_cli, c,
            si->curr_bitrate
        );
        g_object_set(
            si->stream[encoder], "bitrate", si->curr_bitrate,
            NULL
        );
    }
}
...
static void client_close_handler(GstRTSPClient *client, struct stream_info *si)
{
    ...
    if (si->num_cli == 0)
    {
        ...
        if (!(
            si->stream[pipeline] && si->stream[source] &&
...
static void new_client_handler(
    GstRTSPServer *server, GstRTSPClient *client,
...
int main(int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        switch (c)
        {
            ...
            case 'l':
                ...
                {
                    g_print(
                        "Maximum quant-lvl is " MAX_QUANT_LVL
                        ".\n"
                    );
                ...
                {
                    g_print(
                        "Minimum quant-lvl is " MIN_QUANT_LVL
                        ".\n"
                    );
    ...
    if (user_pipeline)
        ...
        snprintf(
            launch, LAUNCH_MAX, "%s name=source0 ! %s%s"
    ...
    gst_rtsp_mount_points_add_factory(
        info.mounts, mount_point,
        info.factory
    );

    /* Create GLIB MainContext */
    info.main_loop = g_main_loop_new(NULL, FALSE);
    auto src_term = g_unix_signal_add(SIGTERM, G_SOURCE_FUNC(TerminationSignalCallback), info.main_loop);
    auto src_int = g_unix_signal_add(SIGINT, G_SOURCE_FUNC(TerminationSignalCallback), info.main_loop);
